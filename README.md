...
### Тестовые примеры по контекстно-зависимым условиям

#### 1) Уникальность имён объявлений (`semantic.err.duplicateDecl`)
...
Ожидаемо: ошибка повторного объявления `Person` на втором объявлении.

#### 2) Уникальность имён полей внутри одного типа (`semantic.err.duplicateField`)
...
Ожидаемо: ошибка повторного поля `name` внутри `User`.

#### 3) Использование только объявленных пользовательских типов (`semantic.err.unknownType`)
...
Ожидаемо: ошибка неизвестного типа `Person`, если в этом же файле нет `interface Person` или `type Person = { ... };`.

#### 4) Корректный пример (без семантических ошибок)
...
Ожидаемо: семантических ошибок нет, AST строится полностью.
...
Дополнительная схема автомата (по вашему рисунку):

![Схема автомата по заданию](docs/lab2/lab2-automaton-comment-fix.png)
