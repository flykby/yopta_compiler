...
### Расширенные тестовые примеры (для отчёта)

#### A) Корректные выражения

| Входная цепочка | Ожидаемый результат |
|---|---|
| `42` | Одна лексема `INTEGER`, синтаксис корректен, ПОЛИЗ: `42`, значение: `42` |
| `(1+2)*(3+4)` | Корректный разбор со скобками, ПОЛИЗ: `1 2 + 3 4 + *`, значение: `21` |
| `2**3**2` | В текущей реализации `**` имеет тот же уровень, что `* / %`; построятся тетрады по правилам парсера, синтаксис корректен |
| `100/5%6` | Корректный разбор операций одного приоритета, ПОЛИЗ: `100 5 / 6 %`, значение: `2` |
| `a1 + b_2 * c$3` | Корректные идентификаторы, тетрады формируются; числовое значение не вычисляется (есть `id`) |

#### B) Лексические ошибки

| Входная цепочка | Что должно произойти |
|---|---|
| `2 + @` | Лексическая ошибка на `@` (недопустимый символ), разбор прекращается, тетрады не строятся |
| `_abc + 1` | Ошибка на первом символе `_` (идентификатор должен начинаться с буквы) |
| `$x + 2` | Ошибка на первом символе `$` (допустим только после первой буквы идентификатора) |

#### C) Синтаксические ошибки

| Входная цепочка | Что должно произойти |
|---|---|
| `1 +` | Ошибка: ожидается операнд после `+` |
| `(2+3` | Ошибка: ожидается закрывающая скобка `)` |
| `2**` | Ошибка: отсутствует правый операнд для `**` |
| `7 8` | Ошибка «лишние символы после выражения» (второе число не присоединено оператором) |
| `()` | Ошибка: пустое подвыражение в скобках |

#### D) Проверка построения ВПП (тетрады / ПОЛИЗ)

| Входная цепочка | Минимальная проверка результата |
|---|---|
| `2+3*4` | Есть тетрады для `*` и `+`; ПОЛИЗ: `2 3 4 * +`; значение: `14` |
| `(27/3)**3+423` | ПОЛИЗ: `27 3 / 3 ** 423 +`; значение: `1152` |
| `x+y*z` | Есть тетрады, но поле «значение» пустое/не вычисляется (идентификаторы) |
